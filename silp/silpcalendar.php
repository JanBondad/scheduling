...
<head>
    ...
    <style>
        ...
        button.disabled {
            ...
            /* This will completely prevent clicking */
        }
    ...
    <body>
        ...
        <div id="calendar">
            ...
            <div class="calendar-header">
                <div class="date-navigation">
                    <a href="silpcalendar.php?month=<?php echo $prevMonth; ?>&year=<?php echo $prevYear; ?>&church=<?php echo $selectedChurch; ?>"
                ...
                <a href="silpcalendar.php?month=<?php echo $nextMonth; ?>&year=<?php echo $nextYear; ?>&church=<?php echo $selectedChurch; ?>"
        ...
        <script>
            ...
            function getValue(time) {
                console.log(availableSlots[time] || 0)
                return availableSlots[time] || 0; // Return the value or 0 if the time is not found
            }

            async function populateTimeSlots(service) {
                var container = document.getElementById('timeSlotContainer');
                container.innerHTML = '<p>Loading available time slots...</p>';
                availableSlots = await checkAvailability(selectedService, selectedDate);
                ...
                fetch(`get_time_slots.php?service=${encodeURIComponent(service)}&date=${encodeURIComponent(selectedDate)}&church=<?php echo $selectedChurch; ?>`)
                    ...
                    .then(timeSlots => {
                        ...
                        Object.entries(timeSlots).forEach(([time, data], index) => {
                    ...
                    <button 
                        ...
                        ${getValue(time) === data.max_slots ? 'disabled' : ''}>
                        ${timeDisplay}
                        <span class="slot-info">
                            (${data.max_slots - getValue(time)} of ${data.max_slots} slots available)
                        </span>
                    </button>`;
            ...
            async function checkAvailability(service, date) {
                try {
                    const response = await fetch(`check_availability.php?date=${date}&service=${service}`);
